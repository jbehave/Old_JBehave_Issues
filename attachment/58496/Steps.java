...
public class Steps implements CandidateSteps {
	...
	private void addVariants(List<StepCandidate> candidates, Method method,
			StepType stepType, String stepAsPatternString, int priority) {
		Set<String> allVariants = new HashSet<String>();
		if (method.isAnnotationPresent(Aliases.class)) {
			String[] variants = method.getAnnotation(Aliases.class).variants();
			allVariants.addAll(Arrays.asList(variants));
		}
		if (method.isAnnotationPresent(Alias.class)) {
			String[] variants = method.getAnnotation(Alias.class).variants();
			allVariants.addAll(Arrays.asList(variants));
		}
		for (String variant : allVariants) {
			addCandidate(candidates, method, stepType, variant + " "
					+ stepAsPatternString, priority);
		}
