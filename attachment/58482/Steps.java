...
import org.jbehave.core.annotations.Variant;
import org.jbehave.core.annotations.Variants;
...
public class Steps implements CandidateSteps {
	 ...
	 */
	public Steps(Configuration configuration, Class<?> type,
	...
	public List<StepCandidate> listCandidates() {
		...
		for (Method method : allMethods()) {
			if (method.isAnnotationPresent(Given.class)) {
				...
				addVariants(candidates, method, GIVEN, value, priority);
			...
			if (method.isAnnotationPresent(When.class)) {
				...
				addVariants(candidates, method, WHEN, value, priority);
			...
			if (method.isAnnotationPresent(Then.class)) {
				...
				addVariants(candidates, method, THEN, value, priority);
	...
	private void addCandidate(List<StepCandidate> candidates, Method method,
		...
		StepCandidate candidate = createCandidate(method, stepType,
	...
	private void checkForDuplicateCandidates(List<StepCandidate> candidates,
	...
	private List<String> getVariants(Method method) {
		List<String> allVariants = new ArrayList<String>();
		if (method.isAnnotationPresent(Variant.class)) {
			allVariants.add(method.getAnnotation(Variant.class).value());
		}
		if (method.isAnnotationPresent(Variants.class)) {
			String[] values = method.getAnnotation(Variants.class).values();
	...
	private void addCandidatesFromAliases(List<StepCandidate> candidates,
		...
		if (method.isAnnotationPresent(Aliases.class)) {
			...
			for (String alias : aliases) {
				...
				addVariants(candidates, method, stepType, alias, priority);
			}
		}
	...
	private StepCandidate createCandidate(Method method, StepType stepType,
			String stepPatternAsString, int priority,
			Configuration configuration) {
		return new StepCandidate(stepPatternAsString, priority, stepType,
				method, type, stepsFactory, configuration.keywords(),
	...
	private List<BeforeOrAfterStep> scenarioStepsHaving(ScenarioType type,
			Stage stage, Class<? extends Annotation> annotationClass,
			Outcome... outcomes) {
		List<BeforeOrAfterStep> steps = new ArrayList<BeforeOrAfterStep>();
		for (Method method : methodsAnnotatedWith(annotationClass)) {
			...
			if (type == scenarioType) {
				...
				if (stage == Stage.AFTER) {
					Outcome scenarioOutcome = scenarioOutcome(method,
							annotationClass);
	...
	private ScenarioType scenarioType(Method method,
			Class<? extends Annotation> annotationClass) {
		if (annotationClass.isAssignableFrom(BeforeScenario.class)) {
			return ((BeforeScenario) method.getAnnotation(annotationClass))
					.uponType();
		}
		if (annotationClass.isAssignableFrom(AfterScenario.class)) {
			return ((AfterScenario) method.getAnnotation(annotationClass))
					.uponType();
		}
		return ScenarioType.NORMAL;
	}

	private Outcome scenarioOutcome(Method method,
			Class<? extends Annotation> annotationClass) {
		if (annotationClass.isAssignableFrom(AfterScenario.class)) {
			return ((AfterScenario) method.getAnnotation(annotationClass))
					.uponOutcome();
	...
	private BeforeOrAfterStep createBeforeOrAfterStep(Stage stage,
			Method method, Outcome outcome) {
		return new BeforeOrAfterStep(stage, method, outcome, new StepCreator(
				type, stepsFactory, configuration.parameterConverters(), null,
	...
	private List<Method> methodsAnnotatedWith(
			Class<? extends Annotation> annotationClass) {
